...
class Pix2Pix3dModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.optimizer_D = torch.optim.Adam(list(self.netD.parameters()),
                                                lr=opt.lr*0.1, betas=(opt.beta1, 0.999))    #multiply 0.1 to discriminator lr
            '''
    ...
    def test(self):
        #torch.cuda.set_device(  int(self.opt.gpu_ids[0]) )  #added
    ...
    def compute_D_only(self):   #revised added
        fake_AB = self.fake_AB_pool.query(torch.cat((self.real_A, self.fake_B), 1))
        self.pred_fake = self.netD.forward(fake_AB.detach())
        self.loss_D_fake = self.criterionGAN(self.pred_fake, False)
    ...
    def optimize_parameters(self):
        ...
        if self.D_back_flag%10 ==0:    #revised to reduce discriminator backprojection
            self.optimizer_D.zero_grad()
            self.backward_D()
            self.optimizer_D.step()
            self.D_back_flag+=1
